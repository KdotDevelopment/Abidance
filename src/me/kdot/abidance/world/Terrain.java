package me.kdot.abidance.world;
...
public class Terrain {
	...
	private static final float SIZE = 6400;
	private static final float MAX_HEIGHT = 1200;
	private static final float MIN_HEIGHT = -1200;
	...
	public float getHeightOfTerrain(float worldX, float worldZ) {
		...
		float answer;
		if (xCoord <= (1-zCoord)) {
			answer = AMath.barryCentric(new Vector3f(0, heights[gridX][gridZ], 0), new Vector3f(1,
							heights[gridX + 1][gridZ], 0), new Vector3f(0,
							heights[gridX][gridZ + 1], 1), new Vector2f(xCoord, zCoord));
		} else {
			answer = AMath.barryCentric(new Vector3f(1, heights[gridX + 1][gridZ], 0), new Vector3f(1,
	...
	private RawModel generateTerrain(Loader loader, String heightMap){
		...
		int VERTEX_COUNT = 512;//image.getHeight();
		...
		for(int i=0;i<VERTEX_COUNT;i++){
			for(int j=0;j<VERTEX_COUNT;j++){
				...
				float height = getHeight(j,i,generator);//getHeight(j,i,image);
				heights[j][i] = height;
				vertices[vertexPointer*3+1] = height;
				vertices[vertexPointer*3+2] = (float)i/((float)VERTEX_COUNT - 1) * SIZE;
				Vector3f normal = calculateNormal(j,i,generator);//calculateNormal(j,i,image);
